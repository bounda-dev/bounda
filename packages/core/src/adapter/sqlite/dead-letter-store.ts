...
export interface CreateSqliteDeadLetterStoreFunction {
  (args: CreateSqliteDeadLetterStoreArgs): DeadLetterStore;
}
  ...
  return {
    ...
    list: async (args = {}) => {
      ...
      const rows = await db.all(
        `SELECT ${COLUMNS} FROM ${table}${where.sql} ORDER BY "first_failed_at", "id" LIMIT ? OFFSET ?`,
        [...where.params, args.limit ?? -1, args.offset ?? 0],
      );
