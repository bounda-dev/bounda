...
/**
 * The part of a Durable Object's `ctx.storage` the adapter uses: its synchronous SQLite handle
...
/**
 * The same shape the SQLite adapter's stores are written against: two statements and write
 * transactions.
 */
export interface DurableSqlDatabase extends SqlExecutor {
  write<T>(work: (tx: SqlExecutor) => Promise<T>): Promise<T>;
}

export interface CreateDurableSqlDatabaseFunction {
  (storage: DurableSqlStorage): DurableSqlDatabase;
}
...
/**
 * Wraps a Durable Object's SQLite storage. `sql.exec` is synchronous; the executor returns
 * resolved promises so the stores keep their asynchronous interface. `write` runs the work inside
 * `storage.transaction`, which rolls back when the work throws.
 */
